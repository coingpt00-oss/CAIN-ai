// src/app/api/public/coin/detail/route.ts
...
// GET /api/public/coin/detail?id=bitcoin&vs=krw|usd
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // CoinPage에서 기대하는 형태: { ok, data, fallback }
    return NextResponse.json(
      {
        ok: true,
        data: payload,
        fallback: false, // Supabase 스냅샷이니까 별도 fallback 개념은 없음
      },
      {
        status: 200,
        headers: {
    ...
    console.error("[coin_detail] exception", e?.message || e);
    return NextResponse.json(
